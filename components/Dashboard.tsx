...
import { createExperiment, fetchExperiments } from '../services/experimentsService';
...
const Dashboard: React.FC = () => {
    ...
    useEffect(() => {
        const loadExperiments = async () => {
            try {
                ...
                setError(err instanceof Error ? err.message : 'Kon experimenten niet laden.');
            } finally {
                setIsLoading(false);
            }
        };
    ...
    const handleCreateExperiment = async (newExpData: any) => {
        try {
            setError(null);
            ...
            setError(err instanceof Error ? err.message : 'Het aanmaken van het experiment is mislukt.');
        }
    };
    ...
    const filteredExperiments = useMemo(() => {
        ...
        return experiments.filter((exp) =>
            ...
            exp.url.toLowerCase().includes(term)
        );
    }, [experiments, search]);

    const totalVisitors = filteredExperiments.reduce((acc, curr) => acc + curr.variants.reduce((vAcc, v) => vAcc + v.visitors, 0), 0);
    const avgConversion = filteredExperiments.length === 0 ? 0 : filteredExperiments.reduce((acc, curr) => {
    ...
    }, 0) / filteredExperiments.length * 100;
    ...
    return (
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white shadow-lg shadow-blue-200">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-blue-100 font-medium mb-1">Bezoekers verwerkt</p>
                            <h3 className="text-3xl font-bold">{totalVisitors.toLocaleString()}</h3>
                            <p className="text-blue-100 text-sm mt-1">Som van alle varianten</p>
                        </div>
                ...
                <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-gray-500 font-medium mb-1">Actieve experimenten</p>
                            <h3 className="text-3xl font-bold text-gray-800">{filteredExperiments.filter(e => e.status === ExperimentStatus.RUNNING).length}</h3>
                            <p className="text-sm text-gray-500">Alleen experimenten met status "Running".</p>
                        </div>
                ...
                <div className="bg-white rounded-xl p-6 border border-gray-200 shadow-sm">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-gray-500 font-medium mb-1">Gemiddelde conversieratio</p>
                            <h3 className="text-3xl font-bold text-gray-800">{avgConversion.toFixed(2)}%</h3>
                            <p className="text-sm text-gray-500">Gebaseerd op alle varianten met bezoekers.</p>
                        </div>
            ...
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
                ...
                >
                    <Plus className="w-4 h-4" /> Nieuw experiment
                </button>
            </div>
            ...
            {isLoading ? (
                <div className="bg-white border border-gray-200 rounded-xl p-10 text-center text-gray-500">Bezig met laden...</div>
            ) : (
                <>
            ...
            {filteredExperiments.length === 0 ? (
                <div className="bg-white border border-gray-200 rounded-xl p-10 text-center text-gray-600">
                    Er zijn nog geen experimenten opgeslagen. Voeg er een toe om statistieken te zien.
                </div>
            ) : (
                        ...
                        <table className="w-full text-left">
                            <thead>
                                <tr className="bg-gray-50 border-b border-gray-200">
                                    ...
                                    <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Bezoekers</th>
                                    <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider">Varianten</th>
                                    <th className="px-6 py-4 text-xs font-semibold text-gray-500 uppercase tracking-wider text-right">Acties</th>
                                </tr>
                            </thead>
                                    ...
                                    <tr key={exp.id} className="hover:bg-gray-50 transition-colors group">
                                        ...
                                        <td className="px-6 py-4">
                                            <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                                ${exp.status === ExperimentStatus.RUNNING ? 'bg-green-100 text-green-800' :
                                                  exp.status === ExperimentStatus.DRAFT ? 'bg-gray-100 text-gray-800' :
                                                  exp.status === ExperimentStatus.COMPLETED ? 'bg-blue-100 text-blue-800' : 'bg-yellow-100 text-yellow-800'}`}>
                                                <span className={`w-1.5 h-1.5 rounded-full mr-1.5 ${
                                                    exp.status === ExperimentStatus.RUNNING ? 'bg-green-500' :
                                                    exp.status === ExperimentStatus.DRAFT ? 'bg-gray-500' :
                                                    exp.status === ExperimentStatus.COMPLETED ? 'bg-blue-500' : 'bg-yellow-500'
                                                }`}></span>
                                                {exp.status}
                                            </span>
                                        </td>
